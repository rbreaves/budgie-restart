...
class KintoApplet(Budgie.Applet):
	...
	def initUI(self):
		...
		# self.img = Gtk.Image.new_from_icon_name("kinto-invert", Gtk.IconSize.BUTTON)
		...
		if res.returncode == 0:
			...
			self.button_suspend.connect("clicked",self.suspend,True)
		else:
			...
			self.button_suspend.connect("clicked",self.suspend,False)
		...
		if True:
			...
			self.button_winmac.connect("clicked",self.setKB,"winmac")
		else:
			self.button_winmac.set_label("Set Mac Only KB Type")
			self.button_winmac.connect("clicked",self.setKB,"mac")
	...
	def setKB(self,button,kbtype):
		kconfig = self.homedir+"/.config/kinto/kinto.py"
		try:
			if kbtype == "winmac":
			...
			self.button_winmac.connect("clicked",self.setKB,connect)
